...
\newtcolorbox{windows}{colback=cyan!5,colframe=blue!55!cyan,boxrule=0.8pt,
  left=8pt,right=8pt,top=6pt,bottom=6pt,title=\textbf{On Windows},fonttitle=\color{white}}
...
{\large VS Code + PlatformIO. Beginner-friendly --- works on \textbf{Windows} and \textbf{Mac}.}\\[6pt]
...
\begin{note}
This guide works on \textbf{Windows} and \textbf{Mac}. The steps are the same on both; a
blue \textbf{On Windows} box appears wherever something differs (the installer, the USB
driver, and the port name). The screenshots are from a Mac, but VS Code looks the same on
Windows.
\end{note}
...
download button for your system (or the \textbf{Download} button at the top-right).}

\begin{windows}
Click \textbf{Download for Windows} --- you'll get a \texttt{.exe} installer. Run it and
accept the defaults (it's fine to leave ``Add to PATH'' checked).
\end{windows}
...
\begin{windows}
Windows often needs this driver. Download the \textbf{CP210x Universal Windows Driver},
\textbf{unzip} it, then right-click the file \texttt{silabser.inf} and choose
\textbf{Install} (or run the included \texttt{CP210xVCPInstaller\_x64.exe}). Then open
\textbf{Device Manager}: under \textbf{Ports (COM \& LPT)} you should see
``Silicon Labs CP210x\ldots (COM\emph{x})''.
\end{windows}
...
\begin{windows}
To confirm Windows sees the board, open \textbf{Device Manager} \(\rightarrow\)
\textbf{Ports (COM \& LPT)} --- you should see a \texttt{COM\emph{x}} entry (e.g.\
\texttt{COM3}). No entry means the driver isn't installed yet (Section~6).
\end{windows}
...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item \textbf{(Windows) Which port is it?} Open \textbf{Device Manager} \(\rightarrow\)
        \textbf{Ports (COM \& LPT)} to see the board's \texttt{COM} number. If nothing
        appears there, the USB driver isn't installed (Section~6).
\end{itemize}
