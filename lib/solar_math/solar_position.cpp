...
SolarAngles calculateSolarPositionRaw(int year, int month, int day,
    ...
    // Panel tracking angle for N-S horizontal single-axis tracker.
    // Negative = tilt east (morning), Positive = tilt west (afternoon).
    float panelAngle = 0.0f;
    if (elevation > 1.0) {
        double azFromSouth = d2r(azimuth - 180.0);
        double elevRad     = d2r(elevation);
        panelAngle = (float)r2d(atan2(cos(elevRad) * sin(azFromSouth), sin(elevRad)));
        panelAngle = fmaxf(PANEL_ANGLE_MIN, fminf(PANEL_ANGLE_MAX, panelAngle));
    }
